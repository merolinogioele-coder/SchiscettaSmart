...
// GESTIONE SALVATAGGIO E VISUALIZZAZIONE PIANI
saveCurrentPlanBtn.addEventListener('click', () => {
  if (!currentPlan || currentPlan.length === 0) return;
  
  const defaultName = `Piano ${capitalize(currentPlan[0].dieta || 'Settimana')} - ${new Date().toLocaleDateString()}`;
  const planName = prompt("Inserisci un nome per questo piano:", defaultName);
  if (!planName) return;

  const savedPlansJson = localStorage.getItem('schiscetta_saved_plans_list');
  let savedPlans = savedPlansJson ? JSON.parse(savedPlansJson) : [];
  ...
  localStorage.setItem('schiscetta_saved_plans_list', JSON.stringify(savedPlans));
  alert("Piano salvato con successo!");
});

viewSavedPlansBtn.addEventListener('click', () => {
...
function renderSavedPlansList() {
  ...
  if (savedPlans.length === 0) {
    savedPlansList.innerHTML = '<p style="color: var(--text-muted); text-align: center; padding: 1rem;">Nessun piano salvato al momento.</p>';
    return;
  }
  ...
  savedPlans.forEach(item => {
    ...
    div.innerHTML = `
      <div>
        <h4 style="color: var(--primary-green); margin: 0; font-size: 1rem;">${item.name}</h4>
      ...
      <div style="display: flex; gap: 0.5rem;">
        ...
        <button class="secondary-btn compact-btn delete-btn" style="padding: 0.4rem 0.8rem !important; font-size: 0.8rem !important; margin: 0 !important; border-color: #dc2626 !important; color: #dc2626 !important; width: auto !important; height: auto !important;">Elimina</button>
      </div>
    `;
    ...
    div.querySelector('.delete-btn').addEventListener('click', () => {
      if (confirm(`Sei sicuro di voler eliminare "${item.name}"?`)) {
        const updatedPlans = savedPlans.filter(p => p.id !== item.id);
        localStorage.setItem('schiscetta_saved_plans_list', JSON.stringify(updatedPlans));
        renderSavedPlansList();
      }
    });
